-- Gerado por npm run db:sql. Migração idempotente; use a conexão administrativa.
BEGIN;
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'agenda_runtime') THEN
    GRANT SELECT, INSERT, UPDATE, DELETE ON agenda_meta, agenda_groups, agenda_tasks, agenda_history, agenda_operations, agenda_conversations, agenda_messages, agenda_limits, agenda_llm_providers, agenda_llm_usage, agenda_security, agenda_sessions, agenda_finance_categories, agenda_finance_entries, agenda_finance_templates, agenda_notes, agenda_note_files TO agenda_runtime;
  END IF;
END $$;
COMMIT;
